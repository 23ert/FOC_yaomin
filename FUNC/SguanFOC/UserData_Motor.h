...
static inline void User_Motor_Init(SguanFOC_System_STRUCT *user){
    ...
    user->foc.Target_Speed = 4.7123890f;            // (float)期望的机械角速度 [对比测试:=2π*3Hz/4极对,与myVF的freq=3Hz电频率对齐]
    user->foc.Target_Pos = 0.0f;                    // (float)期望的机械角度值
    user->foc.Target_Id = 0.0f;                     // (float)期望的D轴电流
    user->foc.Target_Iq = 0.0f;                     // (float)期望的Q轴电流

    user->foc.Target_VF_Uq = 6.0f;                  // (float)设置的Uq大小
...
//     user->motor.VBUS = 12.0f;                       // (float)标定的母线电压
   ...
   user->motor.identify.Rs = 6.97;             // (float)相线电阻_默认参数
   user->motor.identify.Ld = 0.00535f;        // (float)D轴电感_默认参数f
   user->motor.identify.Lq = 0.00535f;        // (float)Q轴电感_默认参数
   
   user->motor.identify.Flux = 0.016884f;        // (float)电机磁链_默认参数
   user->motor.identify.B = 0.00028043f;           // (float)粘性阻尼_默认参数
   user->motor.identify.J = 0.00028043f;           // (float)转动惯量_默认参数
   
    /* ====================================== 分割线 =================================== */
    user->motor.Poles = 4;                          // (uint8_t)电机的极对数
    ...
    user->motor.Duty = 8000;                        // (uint16_t)PWM满占空比数值
