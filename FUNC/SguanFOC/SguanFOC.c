...
static void PWMT_Tick(SguanFOC_System_STRUCT *sguan,
                    float U_alpha,
                    float U_beta);
...
//       (sguan->motor.ADC_Precision*sguan->motor.Amplifier*sguan->motor.Sampling_Rs);
    ...
    sguan->current.Final_Gain = 0.02197f;
    if(uiSampleCnt < 500)
    {
        uiSampleCnt++;
        sguan->current.IZeroSum[0] += ADC_CURRU_REG; // 累加ADC1注入通道1数据（相位A电流零点）
        sguan->current.IZeroSum[1] += ADC_CURRV_REG; // 累加ADC1注入通道2数据（相位B电流零点）
        sguan->current.IZeroSum[2] += ADC_CURRW_REG; // 累加ADC2注入通道1数据（相位C电流零点）
        return;
    }
    else
    {
        ...
        sguan->current.UCurZero = sguan->current.IZeroSum[0]/500; // 计算相位A电流零点平均值
        sguan->current.VCurZero = sguan->current.IZeroSum[1]/500; // 计算相位B电流零点平均值
        sguan->current.WCurZero = sguan->current.IZeroSum[2]/500; // 计算相位C电流零点平均值
        
        sguan->current.IZeroSum[0] = 0;
        sguan->current.IZeroSum[1] = 0;
        sguan->current.IZeroSum[2] = 0;
...
static void Current_ReadIabc(SguanFOC_System_STRUCT *sguan){
    ...
    sguan->current.Real_Ia = (User_ReadADC_Raw(0) - sguan->current.UCurZero)* sguan->current.Final_Gain;
    sguan->current.Real_Ib = (User_ReadADC_Raw(1) - sguan->current.VCurZero)* sguan->current.Final_Gain;
    sguan->current.Real_Ic = (User_ReadADC_Raw(2) - sguan->current.WCurZero)* sguan->current.Final_Gain;
...
static void PWMT_Tick(SguanFOC_System_STRUCT *sguan,
    ...
    float ta = U_alpha;
    float tb = -HALF * U_alpha + INV_SQRT3_2 * U_beta;
    float tc = -HALF * U_alpha - INV_SQRT3_2 * U_beta;
     ...
     */
    float inv_vbus = 1.0f / sguan->motor.VBUS;
    tABC[0] = (-ta * inv_vbus + HALF) * sguan->motor.Duty;
    tABC[1] = (-tb * inv_vbus + HALF) * sguan->motor.Duty;
    tABC[2] = (-tc * inv_vbus + HALF) * sguan->motor.Duty;
    
    User_PwmDuty_Set(tABC[0], tABC[1], tABC[2]);
}
...
void SguanFOC_main_Loop(void){
    ...
    if (!count){
        ...
        // 启用串口DMA接收
       if(HAL_UARTEx_ReceiveToIdle_DMA(&huart3, Sguan_PrintfBuff, sizeof(Sguan_PrintfBuff))!=HAL_OK)
       {
           Error_Handler();
      }
